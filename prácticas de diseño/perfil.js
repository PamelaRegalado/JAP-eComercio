...
function guardar(){
    ...
    sessionStorage.setItem("lista",JSON.stringify(usuario)); //Guardo la lista en localStorage
...
function muestro(){
    var perfil = JSON.parse(sessionStorage.getItem("lista"));
    document.getElementById("muestroPerfil").innerHTML= `<h1>` + perfil.nombre + ` </h1>
    <p>` + perfil.apellido + `</p>
    <p>` + perfil.edad + `</p>
    <p>` + perfil.dirección + `</p>
    <p>` + perfil.telefono + `</p>`;
